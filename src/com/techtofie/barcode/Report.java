			...
			public void actionPerformed(ActionEvent arg0) {
		      ...
		      /* if(ReportType=="Expense Report"){
		       ...
		       if(ReportType=="Transaction Report"){
					try{
						connection=ConnectionUtility.getConnection();
			        String expensedetails = "select BILL_NO,TRX_DATE,IMEI,BRAND,MODEL,COLOUR,DISCOUNT,TOTAL_AMT,DESCRIPTION from barcodebilling.transactions where trx_date between '"+from+"' and '"+to+"'";
					statement=connection.prepareStatement(expensedetails);
					set=statement.executeQuery();
					table.setModel(DbUtils.resultSetToTableModel(set));
					sum2();
...
public  void sum2(){
		...
		for(int i=0;i<table.getRowCount();i++)
		{
			...
			sum=sum+Float.parseFloat(table.getValueAt(i, 7).toString());
